...
export default function Wallet() {
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <GlassCard className="relative overflow-hidden p-10 bg-gradient-to-tr from-secondary/20 via-transparent to-primary/20 border-white/10 group mb-4">
        <div className="absolute top-0 left-0 w-48 h-48 bg-secondary/10 blur-[80px] -ml-24 -mt-24 rounded-full animate-pulse" />
        <div className="absolute bottom-0 right-0 w-32 h-32 bg-primary/10 blur-[60px] -mr-16 -mb-16 rounded-full" />
        
        <div className="text-center">
          <p className="text-[10px] font-black text-white/40 mb-2 uppercase tracking-[0.3em]">{t("balance")}</p>
          <h2 className="text-6xl font-black text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.2)] mb-8 tracking-tighter">
            <span className="text-2xl font-light opacity-50 mr-1">$</span>
            {state.balance.toFixed(2)}
          </h2>
          
          <div className="w-full h-12 bg-black/40 backdrop-blur-md border border-accent-gold/20 rounded-2xl flex items-center px-5 gap-3 shadow-highlight group-hover:scale-105 transition-transform duration-500">
            <CoinIcon size={24} />
            <span className="text-accent-gold font-black text-xl tracking-tight">{state.coins.toLocaleString()}</span>
            <span className="ml-auto text-[10px] font-black text-white/20 uppercase tracking-widest">{t("membership")}</span>
          </div>
        </div>
      </GlassCard>
