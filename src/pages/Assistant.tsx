...
export default function Assistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "model", text: t("assistant_welcome") }
  ]);
    ...
    <div className="flex flex-col h-[calc(100vh-210px)] space-y-4" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
        ...
        >
          {messages.map((m, i) => (
            <motion.div 
              ...
              className={cn(
                "flex gap-3",
                m.role === 'user' ? (isRtl ? "flex-row-reverse" : "flex-row-reverse") : (isRtl ? "flex-row" : "flex-row")
              )}
            >
              <div className={cn(
        ...
        <div className={cn(
          ...
          isFocused ? "mb-16 -translate-y-2 scale-[1.02]" : "mb-0 translate-y-0 scale-100"
        )}>
          {/* Quick Suggestions */}
          <div className={cn(
