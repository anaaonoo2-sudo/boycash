...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        // High-speed admin check first
        setIsAdmin(firebaseUser.email === adminEmail);
        
        // Deep security check (Banned status)
        try {
          ...
          if (userDoc.exists() && userDoc.data().isBanned) {
            ...
            setIsAdmin(false);
            setAuthError({ 
              message: "تم حظر حسابك بسب خرق شروط الاستخدام (VPN أو غش). تواصل مع الدعم للمراجعة.", 
    ...
    // Check for redirect result if needed - handles success and error cases
    const initAuth = async () => {
      try {
        ...
        let message = "فشل تسجيل الدخول التلقائي.";
        if (error.code === 'auth/unauthorized-domain') {
          message = "النطاق غير مصرح به. (Domain Unauthorized)";
        }
